...
`herdr-extractor` is the standalone laktak/extrakto-inspired visible-buffer token picker for Herdr.
...
- `src/extract.rs`: pure visible-buffer token extraction and soft-wrap reconstruction.
- `src/extract_app.rs`: pure typeahead/selection state machine.
- `src/extract_ui.rs`: ratatui renderer.
- `src/herdr_client.rs`: bounded Unix-socket calls for visible text, layout, and notifications.
